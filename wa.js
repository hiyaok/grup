...
const { default: makeWASocket, useMultiFileAuthState, DisconnectReason, delay, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, makeInMemoryStore } = require('@whiskeysockets/baileys');
...
const SESSION_DIR = path.join(__dirname, 'hiyaok_sessions'); // Nama session yang diminta

// Inisialisasi bot Telegram
const bot = new Telegraf(BOT_TOKEN);

// Create store for better message handling
...
bot.command('connect', async (ctx) => {
    ...
    try {
        ...
        // Buat koneksi WhatsApp dengan konfigurasi yang diperbarui untuk desktop
        const sock = makeWASocket({
            ...
            // Browser configuration untuk desktop macOS
            browser: ['Mac OS', 'Desktop', '10.15.7'],
            // Alternative browser configs yang bisa dicoba:
            // browser: ['WhatsApp', 'Chrome', '120.0.6099.0'],
            // browser: ['Ubuntu', 'Chrome', '120.0.6099.0'],
            ...
            if (connection === 'close') {
                ...
                switch (statusCode) {
                    case DisconnectReason.badSession:
                        ...
                        // Hapus session files untuk bad session
                        const userSessionDir = path.join(SESSION_DIR, userId.toString());
                        if (fs.existsSync(userSessionDir)) {
                    ...
                    case DisconnectReason.multideviceMismatch:
                        message = '⚠️ Versi multi-device tidak cocok. Silakan update WhatsApp Anda.';
                        break;
                    default:
                        if (lastDisconnect?.error?.message?.includes('405')) {
                            message = '❌ Metode koneksi tidak diizinkan. Mencoba ulang dengan konfigurasi berbeda...';
                            // Auto retry dengan delay
                            if (shouldReconnect && retryCount < maxRetries) {
                                retryCount++;
                                setTimeout(() => {
                                    console.log(`Retry attempt ${retryCount}/${maxRetries}`);
                                    ctx.reply(`🔄 Mencoba koneksi ulang (${retryCount}/${maxRetries})...`);
                                    // Trigger reconnect
                                    bot.handleUpdate({ 
                                        message: { 
                                            text: '/connect', 
                                ...
                                }, 3000 * retryCount); // Delay bertambah setiap retry
                                return;
                            }
                        } else {
        ...
        if (error.message.includes('ENOTFOUND')) {
            ...
            errorMessage += 'Metode koneksi tidak diizinkan. Coba lagi dalam beberapa saat.';
        } else {
...
// Launch bot
console.log('🚀 Starting Hiyaok WhatsApp Bot (Desktop Version)...');
console.log('📱 Configured for macOS/iOS Desktop');
bot.launch({
    ...
    console.log('💻 Platform: Desktop (macOS/iOS)');
